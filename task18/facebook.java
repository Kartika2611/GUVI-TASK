	...
	public static void main(String[] args) throws InterruptedException 
		{
			// Creating an object for  driver
			 ...
			 driver.findElement(By.name("firstname")).sendKeys("Hari");//identifying elements on webpage to send first name
			 driver.findElement(By.name("lastname")).sendKeys("karthi");//identifying elements on webpage to send last name
			 driver.findElement(By.name("reg_email__")).sendKeys("Harikartika11@gmail.com"); //sending the email to email field
			 driver.findElement(By.name("reg_email_confirmation__")).sendKeys("Harikartika11@gmail.com");//sending the email to email field for confrimation
			 driver.findElement(By.id("password_step_input")).sendKeys("Suruthy05@12"); //sending the password to password filed
			 WebElement day	= driver.findElement(By.id("day"));//select the values using select class 
			 Select selectday = new Select(day);
			 selectday.selectByVisibleText("11");   //using select class select day 11 by visibletext method 
			 WebElement month = driver.findElement(By.id("month"));         
			 Select selectmonth = new Select(month);
			 selectmonth.selectByValue("5");//using select class select month by value
			 WebElement year = driver.findElement(By.id("year"));
			 Select selectyear = new Select(year);
			 selectyear.selectByVisibleText("1985");//using select class select year by visibletext	 
			 Thread.sleep(2000);
